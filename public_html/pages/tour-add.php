<?php
/**
 * Created by PhpStorm.
 * Project: tourcompany.loc.
 * File: tour-add.php.
 * Date: 24.04.2018
 * Time: 18:22
 */
session_start() ?>
<?php $title = "Добавление тура" ?>
<?php
require_once('../forms/TourForm.php');
require_once('../DB.php');
require_once('../Password.php');
require_once('../Session.php');
require_once('../Dbsettings.php');
$msg = '';
$db = new DB($host, $user, $password, $db_name);
$form = new TourForm($_POST);
if ($_POST) {
    if ($form->validate()) {
        $name = $db->escape($form->getName());
        $description = $db->escape($form->getDescription());
        $category = $db->escape($form->getCategory());
        $cost = $db->escape($form->getCost());

        $email = $_SESSION['email'];
        $res = $db->query("SELECT role_idrole FROM `user` WHERE email = '{$email}'");
        $a = $res[0]['role_idrole'];
        if ($a == 1 || $a == 2) {
            $db->query("INSERT INTO tour (`name`, description, category_idcategory, cost) VALUES ('{$name}', '{$description}', '{$category}','{$cost}') ");
            header('location: tour.php?msg=Тур успешно добавлен!');
        } else {
            $msg = 'У Вас нет прав на добавление категории!';
        }
    } else {
        $msg = 'Пожалуйста, заполните все поля';
    }
}
?>
<?php include 'header.php' ?>
<hr>
<h5 align="center">Менеджер задач</h5>
<hr>
<div class="container">
    <div class="row">
        <div class="col-sm-3">
            <h5 class="text-center border border-top-0 border-left-0" style="line-height: 40px;">Меню</h5>
        </div>
        <div class="col-sm">
            <h5 class="text-center border border-top-0 border-right-0"
                style="line-height: 40px;"><?php echo $title ?></h5>
        </div>
    </div>
    <div class="row">
        <?php include_once('../navigation.php'); ?>
        <div class="col-sm">
            <div class="text-justify border border-bottom-0 border-right-0"
                 style="line-height: 40px; padding-left: 10px; padding-right: 10px;">

                <b style="color: red;"><?= $msg; ?></b>
                <form method="post">
                    <div class="form-group">
                        <label for="name">Название тура</label>
                        <input type="text" class="form-control" id="name" placeholder="Добавьте название тура"
                               name="name"
                               value="<?= $form->getName(); ?>">
                    </div>
                    <div class="form-group">
                        <label for="description">Описание тура</label>
                        <input type="text" class="form-control" id="description"
                               placeholder="Добавьте описание тура"
                               name="description" value="<?= $form->getDescription() ?>">
                    </div>

                    <div class="form-group">
                        <label for="category">Категория</label>
                        <select class="form-control" name="category" id="category">
                            <?php
                            $category = $db->query("SELECT * FROM category");
                            foreach ($category as $categoryitem) {
                                ?>
                                <option value="<?php echo $categoryitem['idcategory'] ?>"><?php echo $categoryitem['namecategory'] ?></option>
                            <?php } ?>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="cost">Стоимость</label>
                        <input type="text" class="form-control" id="cost"
                               placeholder="Добавьте стоимость тура"
                               name="cost" value="<?= $form->getCost() ?>">
                    </div>

                    <button type="submit" class="btn btn-info">Сохранить</button>
                    <a href="tour.php" class="btn btn-info">Отмена</a>

                </form>
            </div>
        </div>
    </div>

</div>


<?php include 'footer.php' ?>
