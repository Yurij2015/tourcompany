...
<?php
if (!empty($_GET['findtour'])) {
    $findtour = trim(htmlspecialchars($_GET['findtour']));
    $categoryname = trim(htmlspecialchars($_GET['category']));
}
?>

<h5 align="center" style="margin-top: 15px;">Найденные туры в категории "<?= $categoryname ?>"</h5>
<hr>
<div class="container">
        ...
        <div class="col-sm" style="padding-bottom: 100px;">
            <div
                ...
                <table class="table table-bordered">
                    ...
                    <?php
                    try {
                    $tour = $db->query("SELECT * FROM tour, category WHERE tour.category_idcategory = category.idcategory AND tour.category_idcategory = '{$findtour}' ");
                    foreach ($tour
